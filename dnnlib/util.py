...
#--------------------------------------------------------------------------
#general helpers (both exp types)
...
def calc_Mahalanoubis(Gamma_inv, X, mu):
    """
    Computes Mahalanoubis distance for a given 
    set of data pts and a reference distribution
    with mean mu and covariance Gamma.
    """
    X = np.expand_dims(X, axis=-1) #bs, dim, 1
    xj = np.expand_dims(np.tile(np.expand_dims(mu, axis=0), (X.shape[0], 1)), axis=-1) #bs, dim, 1
    X_diff = X - xj
    dM = np.einsum('bki, ij -> bkj', np.transpose(X_diff, (0, 2, 1)), Gamma_inv) #bs, 1, dim 
    dM = np.einsum('bki, bij -> bkj', dM, X_diff) #bs, 1, 1
    dM = np.sqrt(np.squeeze(dM))
    return dM
...
#-------------------------------------------------------------------------------
# geometry agnostic experiments helpers 

def sampling_fn_wrapper(**kwargs):
    """
    Wrapper to do sampling in more flexible manner in our 
    geometry-agnostic exps.
    Essentially allows us to sample either from toy dataset directly
    or from a specified Gaussian latent space.
    """
    opts = EasyDict(kwargs)
    if opts.int_mode=='melt':
        #take samples and labels 
        samples = get_toy_dset(opts.dset_name, n=opts.n) 
    else:
        #take samples and std_tot_dims 
        samples = get_gen_samples(opts.data_dims, opts.dims_to_keep, \
                                  opts.device, (opts.n, opts.data_dims), eps=opts.eps)
    return samples 

def sample_rc_testpts(center, max_bpt_radius, d, sampling_fn, sampling_fn_kwargs):
    """
    Constructs sample test pts to be used in geometry-agnostic exps. 
    Does so for a single given random center. 
    
    Args: 
    ------
    center: np.array. location of random center we are sampling test pts for.
    
    max_samples: int. Max num of samples to take using sampling_fn. These will be 
    chosen as test pts only if their Mahal distance falls within given max_bpt_radius
    from chosen center.
    
    max_bpt_radius: float. Max radius we will use to include/exclude test pts for a
    given random center. 
    
    d: int. Dimensionality of our dataset. 
    
    sampling_fn: python function. Should be either get_gen_samples or get_toy_dset
    depending on whether we are running sim in gen or melt direction respectively. 
    
    sampling_fn_kwargs: dict. Dictionary containing arugments needed for specified 
    sampling_fn. 
    """        
    curr_maxsample_set = sampling_fn(**sampling_fn_kwargs)[0] #get only samples
    
    #pass these to numpy as needed
    if torch.is_tensor(curr_maxsample_set):
        curr_maxsample_set = curr_maxsample_set.cpu().numpy()
        
    #get gamma_inv for dM calculation 
    if 'dset_name' in list(sampling_fn_kwargs.keys()):
        #this is just a melt...
        gamma_inv = np.eye(d)
    else: 
        #this is a gen case ...
        if d == sampling_fn_kwargs['dims_to_keep']:
            #PRP case
            gamma_inv = np.eye(d)
        else: 
            #PRR case 
            gamma_inv = np.ones(sampling_fn_kwargs['dims_to_keep'])
            gamma_inv = np.append(gamma_inv, (np.ones(d - sampling_fn_kwargs['dims_to_keep'])*sampling_fn_kwargs['eps']))
            gamma_inv = np.diag((1/gamma_inv))   
    
    #calc Mahal distances 
    dM_curr_maxsample_set = calc_Mahalanoubis(gamma_inv, curr_maxsample_set, \
                                              center)
    testpts_inside = np.where(dM_curr_maxsample_set < max_bpt_radius, 1, 0)
    testpts_inside = curr_maxsample_set[testpts_inside.astype(bool), :]    
    
    
    return testpts_inside, testpts_inside.shape[0]

def compute_decisions_counts(samples_dM, radius):
    """
    Helper method to compute in/out counts and percentiles
    for a given threshold/radius and given a set of pre-computed
    Mahal distances for samples. 
    
    Args
    -----
    samples_dM: np.array. Contains Mahalanoubis distances for our samples
    radius: float. Decision threshold to use when judgding whether a sample
    pt falls inside or outside a given boundary.
    
    Returns
    ------
    nested dict containing total counts and percent coverages for 
    given radius/threshold and samples.
    """
    decisions = np.where(samples_dM <= radius, 1, 0)
    in_curr_radius = np.sum(decisions)
    out_curr_radius = samples_dM.shape[0] - in_curr_radius
    pin_curr_radius = (in_curr_radius/samples_dM.shape[0])*100
    pout_curr_radius = (out_curr_radius/samples_dM.shape[0])*100
        
    curr_radius_results = {'counts_in':int(in_curr_radius), 
                               'counts_out':int(out_curr_radius), 
                               'percent_in':float(pin_curr_radius), 
                               'percent_out':float(pout_curr_radius)}    
    
    return curr_radius_results, decisions

def compute_preODE_coverages(init_samples, init_center, bpt_radii, init_gamma_scale, verbose=False):
    """
    Computes coverages within each boundary/radius for initial tsample pts (before ODE simulation)
    for geometry-agnostic experiments.
    
    Args
    ----
    init_samples: np.array. Sample pts before simulating ODE.
    init_center: np.array. Coords for center pt before simulating ODE. 
    bpt_radii: list. Contains radii vals for different boundaries constructed. 
    init_gamma_scale: np.array. Stds (per dimension) for initial state. Varies depending
    on schedule type and sim type.
    verbose: bool. If true, will print out a table containing original/initial coverages
    for each boundary.
    """        
    gamma_inv = np.diag(1/(init_gamma_scale**2))
    init_samples_dM = calc_Mahalanoubis(gamma_inv, init_samples, init_center)
    
    init_coverages = {}
    init_decisions = []
    
    for r in bpt_radii:
        curr_radius_results, curr_decisions = compute_decisions_counts(init_samples_dM, r)
        init_coverages[float(r)] = curr_radius_results
        init_decisions.append(curr_decisions)
    
    if verbose: 
        print_coverage_table(init_coverages)
    
    return init_coverages, np.array(init_decisions)

def compute_postODE_coverages(end_samples, end_center, end_bpts, bpt_radii, end_gamma_scale, \
                              bpts_per_radius=200, verbose=False):
    """
    Computes coverages within each boundary/radius for ending (postODE sim) 
    for geometry-agnostic experiments.
    These are computed using min, max, and mediun dM radii. 
    In main appendix results, we only use max radii derived counts.
    
    Args
    ----
    end_samples: np.array. Test pts at end of ODE sim
    end_center: np.array. Center pt at end of ODE sim
    end_bpts: np.array. Bpts at end of ODE sim
    bpt_raddi: list. Containts radii vals for different boudnaries used.
    end_gamma_scale: np.array. Stds (per dimension) for iend/postODE state. Varies depending
    on schedule type and sim type.
    bpts_per_radius: int. Number of bpts sampled per each boundary radius. Defaults to 200.
    verbose: bool. If true, print table showing postODE coverages under median radius metric.
    """
    
    gamma_inv = np.diag(1/(end_gamma_scale**2))
    end_samples_dM = calc_Mahalanoubis(gamma_inv, end_samples, end_center)
    
    #get idxs for bounrady pts 
    idxs = np.arange(0, bpts_per_radius*len(bpt_radii), bpts_per_radius)
    
    #init coverage dictionaries 
    max_end_coverages = {}
    min_end_coverages = {}
    median_end_coverages = {}
    
    #now get coverages using min, max, and median dM radius for each boundary... 
    for i in range(idxs.shape[0]):
        curr_bpts = end_bpts[idxs[i]:(idxs[i]+bpts_per_radius), :]
        curr_bpts_dM = calc_Mahalanoubis(gamma_inv, curr_bpts, end_center)
        curr_bpts_rmin, curr_bpts_rmax, curr_bpts_rmedian = np.amin(curr_bpts_dM), \
            np.amax(curr_bpts_dM), np.median(curr_bpts_dM)
        
        #get coverages for current radius using min/max/mediam dM radii
        rmin_curr_results, _= compute_decisions_counts(end_samples_dM, curr_bpts_rmin)
        rmax_curr_results, _= compute_decisions_counts(end_samples_dM, curr_bpts_rmax)
        rmedian_curr_results, _= compute_decisions_counts(end_samples_dM, curr_bpts_rmedian)
        
        #pass these to general min, max, median dicts
        min_end_coverages[float(bpt_radii[i])] = rmin_curr_results
        max_end_coverages[float(bpt_radii[i])] = rmax_curr_results
        median_end_coverages[float(bpt_radii[i])] = rmedian_curr_results
        
    
    #get global dict with all metrics
    all_postODE_results = {'rmin':min_end_coverages, 
                           'rmax':max_end_coverages, 
                           'rmedian':median_end_coverages}
    
    #print median coverages if desired 
    if verbose: 
        print_coverage_table(median_end_coverages)
    
    return all_postODE_results


def compute_ROC_counts_rates(gt_decision, end_samples, end_center, end_bpts, bpt_radii, \
                             end_gamma_scale, bpts_per_radius=200, eps=1e-7):
    """
    Computes TP, TN, FP, FN counts and FPR/TPR needed to construct ROC curves
    for end of ODE simulation.
    These use rmax as reference for postODE decisions/thresholding.
    Args
    ----
    gt_decision: np.array. Contains original decisions (0's 1's) for given test pts
    and given boundary being assessed.
    end_samples: np.array. Test pts at end of ODE sim
    end_center: np.array. Center pt at end of ODE sim
    end_bpts: np.array. Bpts at end of ODE sim   
    bpt_raddi: list. Containts radii vals for different boudnaries used.
    ...
    on schedule type and sim type.
    bpts_per_radius: int. Number of bpts sampled per each boundary radius. Defaults to 200.
    eps: small fudge factor to avoid div by zero when computing rates. Defaults to 1e-7.
    """
    gamma_inv = np.diag(1/(end_gamma_scale**2))
    end_samples_dM = calc_Mahalanoubis(gamma_inv, end_samples, end_center)
    ...
    for i in range(idxs.shape[0]):    
        ...
        #get rmax for this boundary
        curr_bpts = end_bpts[idxs[i]:(idxs[i]+bpts_per_radius), :]
        curr_bpts_dM = calc_Mahalanoubis(gamma_inv, curr_bpts, end_center)
        curr_bpts_rmax = np.amax(curr_bpts_dM)
        
        #compute decisions based on curr_rmax
        _, curr_bpt_decisions = compute_decisions_counts(end_samples_dM, curr_bpts_rmax)
        
        #compute ROC counts and rates
        #counts
        curr_auc_decisions = 2*gt_decision + curr_bpt_decisions
        tps = np.sum(np.where(curr_auc_decisions==3, 1, 0))
        tns = np.sum(np.where(curr_auc_decisions==0, 1, 0))
        fps = np.sum(np.where(curr_auc_decisions==1, 1, 0))
        fns = np.sum(np.where(curr_auc_decisions==2, 1, 0))
        all_counts.append(np.array([tps, tns, fps, fns]))
        
        #rates
        tpr = (tps/((tps + fns)+eps))
        fpr = (fps/((fps + tns)+eps))
        all_rates.append(np.array([fpr, tpr]))
        
    return np.array(all_counts), np.array(all_rates)
        
def compute_AUCs(rc_roc_rates, bpts_radii):
    """
    Computes AUC (using trapz numerical integration)
    for a given random center. Assumes rrandom center ROC rates
    were already computed using above method.
    """
    rc_aucs = []
    for i in range(bpts_radii.shape[0]):
        curr_radius_auc = trapz(rc_roc_rates[i, :, 1], \
                                rc_roc_rates[i, :, 0])
        rc_aucs.append(curr_radius_auc)
    return rc_aucs
