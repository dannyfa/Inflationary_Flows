...
def training_loop(
    ...
    # Load dataset.
    #Taking this out since we can sample our pts as we go!
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                samples = dnnlib.util.get_toy_dset(dataset_kwargs.dset_name, batch_gpu, augment_to=dataset_kwargs.augment_to)[0] #sample 2/3D data or aug data 
                if 'pca_trf' in dataset_kwargs:
                    #this is ifs/edm_es precond and we should pass samples to ES 
                    samples = dataset_kwargs.pca_trf.transform(samples) 
                samples = torch.from_numpy(samples).to(device).to(torch.float32) #pass images to torch, device
                loss = loss_fn(net=ddp, samples=samples) #bs, dim 
        ...
        #this should yield lighter TB logs 
